...
消息：BINLOG语句中仅允许Format_description_log_event和行事件（但提供了％s）
...
消息：当@@ GLOBAL.GTID_MODE = ON时，不能将@@ SESSION.GTID_NEXT设置为匿名。
...
消息：使用主服务器的SERVER_UUID，从服务器具有比主服务器更多的GTID。这可能表明二进制日志的末尾已被截断，或者最后一个二进制日志文件已丢失，例如，在sync_binlog！= 1的电源或磁盘故障之后。主数据库可能已回滚或可能未回滚已复制的事务。给奴隶 建议将主服务器已从从服务器回滚的所有事务复制到主服务器，和/或在主服务器上提交空事务，以解决已在主服务器上提交但未包含在GTID_EXECUTED中的事务。
...
消息：DROP DATABASE失败；某些表可能已删除，但数据库目录仍然保留。GTID尚未添加到GTID_EXECUTED，并且该语句未写入二进制日志。对此进行如下修复：（1）从数据库目录％s中删除所有文件；（2）SET GTID_NEXT ='％s'; （3）删除数据库'％s'。
...
消息：由于该组已经在运行，因此START GROUP_REPLICATION命令失败。

------

错误号：3094; 符号： ER_GROUP_REPLICATION_APPLIER_INIT_ERROR; SQLSTATE：HY000
消息：由于应用程序模块无法启动，因此START GROUP_REPLICATION命令失败。

------

错误号：3095; 符号： ER_GROUP_REPLICATION_STOP_APPLIER_THREAD_TIMEOUT; SQLSTATE：HY000
消息：STOP GROUP_REPLICATION命令执行未完成：应用程序线程在繁忙时收到停止信号。当前任务完成后，应用程序线程将停止。
...
消息：虚拟列的INPLACE ADD或DROP不能与其他ALTER TABLE操作结合使用
...
该帐户已用CREATE USER ... ACCOUNT LOCK或 锁定 ALTER USER ... ACCOUNT LOCK。管理员可以使用对其进行解锁 ALTER USER ... ACCOUNT UNLOCK。
...
消息：请不要修改％s表。这是一个mysql内部系统表，用于存储已提交事务的GTID。修改它可能导致GTID状态不一致。
...
消息：锁定服务锁名称'％s'不正确。
锁定服务名称指定为NULL，空字符串或长度超过64个字符的字符串。命名空间和锁名称必须为非NULL，非空，并且长度不能超过64个字符。
...
消息：参数％s超出了函数％s中几何图形（％lu）的最大点数。

------

错误号：3135; 符号： ER_SQL_MODE_MERGED; SQLSTATE： HY000
消息：“ NO_ZERO_DATE”，“ NO_ZERO_IN_DATE”和“ ERROR_FOR_DIVISION_BY_ZERO” sql模式应与严格模式一起使用。它们将在以后的版本中与严格模式合并。

------

错误号：3136; 符号： ER_VTOKEN_PLUGIN_TOKEN_MISMATCH; SQLSTATE：HY000
消息：％。* s的版本令牌不匹配。正确值％。* s
客户端已将其version_tokens_session系统变量设置 为需要服务器匹配的令牌列表，但是服务器令牌列表具有至少一个匹配的令牌名称，该名称的值不同于客户端所需的值。请参见 第5.6.6节“版本令牌”。

------

错误号：3137; 符号： ER_VTOKEN_PLUGIN_TOKEN_NOT_FOUND; SQLSTATE：HY000
消息：找不到版本令牌％。* s。
客户端已将其version_tokens_session系统变量设置 为需要服务器匹配的令牌列表，但是服务器令牌列表至少缺少这些令牌之一。请参见第5.6.6节“版本令牌”。

------

错误号：3138; 符号： ER_CANT_SET_VARIABLE_WHEN_OWNING_GTID; SQLSTATE：HY000
消息：拥有GTID的客户端无法更改变量％s。客户拥有％s。所有权在COMMIT或ROLLBACK上释放。
...
消息：无效的JSON文本：列“％s”中位于值％u处的“％s”。

------

错误号：3141; 符号： ER_INVALID_JSON_TEXT_IN_PARAM; SQLSTATE：22032
消息：函数％s中的参数％u中的无效JSON文本：位置％u。％s处的“％s”
...
消息：无法从具有CHARACTER SET'％s'的字符串创建JSON值。
...
消息：函数％s的参数％u中的JSON数据的数据类型无效；JSON字符串或JSON类型是必需的。
...
消息：路径表达式必须用utf8字符集编码。路径表达式“％s”编码为字符集“％s”。
...
讯息：第％ld行的CAST到％s％s的CAST超出范围JSON值

------

错误号：3156; 符号： ER_INVALID_JSON_VALUE_FOR_CAST; SQLSTATE：22018
消息：CAST无效的JSON值从第％ld行的％s列到％s％s
...
消息：JSON文档不能包含NULL成员名称。

------

错误号：3159; 符号： ER_SECURE_TRANSPORT_REQUIRED; SQLSTATE：HY000
消息：--require_secure_transport = ON时，禁止使用不安全的传输进行连接。
...
该require_secure_transport 如果服务器不支持至少一个安全可靠的运输系统变量无法启用。使用必需的SSL密钥/证书配置服务器以启用SSL连接，或启用shared_memory系统变量以启用共享内存连接。

------

错误号：3161; 符号： ER_DISABLED_STORAGE_ENGINE; SQLSTATE：HY000
消息：存储引擎％s已禁用（不允许创建表）。
试图使用disabled_storage_engines系统变量值中列出的存储引擎来创建表或表空间 ，或将现有表或表空间更改为此类引擎。选择其他存储引擎。
...
消息：被审计API（'％s';％d）中止。
此错误表明审核插件终止了事件的执行。该消息通常指示事件子类名称和数字状态值。
...
消息：客户端拥有GTID％s的所有权。因此，WAIT_FOR_EXECUTED_GTID_SET无法等待此GTID。
...
消息：请不要用XA事务修改％s表。这是一个内部系统表，用于存储已提交事务的GTID。尽管修改它可能导致GTID状态不一致，但是如果需要，您可以使用非XA事务进行修改。
...
消息：函数'％s'失败，因为基础密钥环服务返回了错误。请检查是否安装了密钥环插件，并且所提供的参数对您使用的密钥环有效。

------

错误号：3189; 符号： ER_USER_COLUMN_OLD_LENGTH; SQLSTATE：HY000
消息：看来您的数据库架构是旧的。％s列的长度为77个字符，应为93个字符。请执行MySQL升级程序。

------

错误号：3190; 符号： ER_CANT_RESET_MASTER; SQLSTATE： HY000
消息：由于％s，不允许使用RESET MASTER。

------

错误号：3191; 符号： ER_GROUP_REPLICATION_MAX_GROUP_SIZE; SQLSTATE：HY000
消息：由于该组已有9个成员，因此START GROUP_REPLICATION命令失败。
...
消息：无法完成该操作，因为该表已被另一个连接引用。

------

错误号：3197; 符号： ER_XA_RETRY; SQLSTATE： HY000
消息：资源管理器当前无法提交事务分支。请稍后重试。
...
消息：语句不安全，因为它在XA事务中使用。使用语句复制时，并发XA事务可能在从属服务器上死锁。
...
消息：ROW_FORMAT = COMPRESSED或KEY_BLOCK_SIZE不允许使用CREATE TEMPORARY TABLE。

------

错误号：3501; 符号： ER_ACL_OPERATION_FAILED; SQLSTATE： HY000
消息：由于来自SE的以下错误，ACL操作失败：错误代码％d-％s

------

错误号：3502; 符号： ER_UNSUPPORTED_INDEX_ALGORITHM; SQLSTATE：HY000
消息：此存储引擎不支持％s索引算法，而是使用了存储引擎默认值。
...
消息：SRID％u的空间参考系统定义未指定强制性％s（EPSG％u）投影参数。
...
消息：函数％s仅为笛卡尔空间参考系统定义，但其参数之一在SRID％u中，而不是笛卡尔。

------

错误号：3521; 符号： ER_SRS_NOT_CARTESIAN_UNDEFINED; SQLSTATE：SR001
消息：仅为笛卡尔空间参考系统定义函数％s，但其参数之一在SRID％u中，尚未定义。
...
消息：无法获取指定的URN：％s中的模式“％s”的方案加载服务实现。
...
消息：在组件卸载期间，由组件'％s'提供的服务实现'％s'的注册失败。
...
消息：窗口'％s'：ORDER BY或PARTITION BY使用不支持的旧位置指示，请使用表达式。
...
消息：窗口函数'％s'忽略窗口'％s'的frame子句，并在整个分区上聚合
ER_WINDOW_FUNCTION_IGNORES_FRAME 在8.0.2中添加。

------

错误号：3600; 符号： ER_WINDOW_SE_NOT_ACCEPTABLE; SQLSTATE：HY000
消息：开窗需要@@ internal_tmp_mem_storage_engine = TempTable。
ER_WINDOW_SE_NOT_ACCEPTABLE 在8.0.2中添加了，在8.0.3之后删除。
...
消息：存储引擎无法删除表'％s'，因为它已丢失。使用DROP TABLE IF EXISTS将其从数据字典中删除。
...
消息：删除数据库时出现问题。无法删除数据库目录（％s）。请手动将其删除。
...
消息：在操作过程中执行并发DDL。请再试一遍。
ER_DDL_IN_PROGRESS 在8.0.3中添加了，在8.0.16之后删除了。

------

错误号：3633; 符号： ER_CLONE_DDL_IN_PROGRESS; SQLSTATE：HY000
消息：在克隆操作期间执行了并行DDL。请再试一遍。
...
ER_TOO_MANY_CONCURRENT_CLONES 在8.0.3中添加了，在8.0.16之后删除了。
...
消息：％u迭代后中止了递归查询。尝试将@@ cte_max_recursion_depth增加到更大的值。
...
消息：％s资源组％s的无效线程优先级值％d。允许的范围是[％d，％d]。
...
消息：模式目录'％s'已经存在。必须手动解决此问题（例如，通过将架构目录移动到另一个位置）。
...
消息：选项expire_logs_days和binlog_expire_logs_seconds不能一起使用。请使用binlog_expire_logs_seconds设置到期时间（不建议使用expire_logs_days）
ER_BINLOG_EXPIRE_LOG_DAYS_AND_SECS_USED_TOGETHER 已在8.0.11中添加。

------

错误号：3684; 符号： ER_REGEXP_STRING_NOT_TERMINATED; SQLSTATE：HY000
消息：输出字符串不能以零结尾，因为长度超过了缓冲区容量。
ER_REGEXP_STRING_NOT_TERMINATED 在8.0.4中添加了，在8.0.4之后删除了。
...
消息：空间参考系统名称不能为空字符串，也不能以空格开头或结尾。
...
消息：'utf8'当前是字符集UTF8MB3的别名，但在将来的发行版中将成为UTF8MB4的别名。为了明确起见，请考虑使用UTF8MB4。
ER_DEPRECATED_UTF8_ALIAS 已在8.0.11中添加。

------

错误号：3720; 符号： ER_DEPRECATED_NATIONAL; SQLSTATE： HY000
消息：NATIONAL / NCHAR / NVARCHAR表示字符集UTF8MB3，在将来的版本中将由UTF8MB4代替。为了明确起见，请考虑使用CHAR（x）CHARACTER SET UTF8MB4。
ER_DEPRECATED_NATIONAL 已在8.0.11中添加。

------

错误号：3721; 符号： ER_INVALID_DEFAULT_UTF8MB4_COLLATION; SQLSTATE：HY000
消息：无效的默认归类％s：预期为utf8mb4_0900_ai_ci或utf8mb4_general_ci
...
消息：当会话具有打开的临时表时，不允许更改@@ session.binlog_format。您可以等到删除这些临时表后再试一次。
...
消息：当任何复制通道具有打开的临时表时，不允许更改@@ global.binlog_format或@@ persist.binlog_format。您可以等到Slave_open_temp_tables = 0再试一次
...
消息：任何复制通道应用程序线程正在运行时，不允许更改@@ global.binlog_format或@@ persist.binlog_format。您可以执行STOP SLAVE SQL_THREAD，然后重试。
...
消息：语句违反了GTID一致性：@@ session.binlog_format = STATEMENT时，不允许在事务内部或事务上下文中的过程内部创建CREATE TEMPORARY TABLE和DROP TEMPORARY TABLE。
ER_CLIENT_GTID_UNSAFE_CREATE_DROP_TEMP_TABLE_IN_TRX_IN_SBR 已在8.0.13中添加。

------

错误号：3750; 符号： ER_TABLE_WITHOUT_PK; SQLSTATE： HY000
消息：设置系统变量'sql_require_primary_key'时，无法创建或更改没有主键的表。将主键添加到表中或取消设置此变量以避免出现此消息。请注意，没有主键的表可能会导致基于行的复制中的性能问题，因此在更改此设置之前，请咨询您的DBA。
ER_TABLE_WITHOUT_PK 已在8.0.13中添加。

------

错误号：3751; 符号： WARN_DATA_TRUNCATED_FUNCTIONAL_INDEX; SQLSTATE：01000
消息：第％ld行的功能索引'％s'的数据被截断
WARN_DATA_TRUNCATED_FUNCTIONAL_INDEX 在8.0.13中添加了，在8.0.16之后删除。
...
消息：无法在返回JSON或GEOMETRY值的函数上创建函数索引。
...
消息：无法删除列'％s'，因为它由功能索引使用。为了删除该列，必须删除功能索引。
...
消息：无法在返回BLOB或TEXT的表达式上创建函数索引。请考虑使用CAST。
...
消息：语句违反了GTID一致性：ALTER TABLE ... ADD COLUMN ..表达式为DEFAULT。
...
消息：更改二进制日志加密设置后，无法旋转某些日志。请解决问题并手动旋转日志。
...
消息：无法恢复Binlog加密主密钥，请检查是否已加载密钥环插件。
...
消息：慢速查询日志文件格式已按要求更改，但是当未实际登录到文件时，该设置将无效。
...
消息：选项group_replication_consistency不能在当前成员状态下使用。
...
消息：无法在密钥环上操作二进制日志主密钥，请检查是否已加载密钥环插件。该语句无效：旧的二进制日志主密钥仍在使用，密钥环，二进制和中继日志文件未更改，服务器无法开始使用新的二进制日志主密钥对新的二进制和中继日志文件进行加密。
...
消息：无法旋转一个或多个二进制或中继日志文件。生成了一个新的二进制日志主密钥，该密钥将用于加密新的二进制和中继日志文件。使用先前的二进制日志主密钥，仍可能存在二进制或中继日志文件。
...
讯息：％s。生成了一个新的二进制日志主密钥，该密钥将用于加密新的二进制和中继日志文件。使用先前的二进制日志主密钥，仍可能存在二进制或中继日志文件。
...
消息：无法从密钥环中删除未使用的二进制日志加密密钥，请检查是否已加载密钥环插件。未使用的二进制日志加密密钥可能仍存在于密钥环中，并且在服务器重新启动或下次执行“ ALTER INSTANCE ROTATE BINLOG MASTER KEY”时将其删除。
...
消息：无法从密钥环中删除辅助二进制日志加密密钥，请检查是否已加载密钥环插件。二进制日志主密钥轮换过程的清理未按预期完成，并且清理将在服务器重新启动或下一次执行“ ALTER INSTANCE ROTATE BINLOG MASTER KEY”时进行。
...
消息：传递给函数％s的几何位于SRID 0中，该ID未指定长度单位。无法转换为“％s”。
...
消息：无法更改表'％s'的列'％s'。索引'％s'的结果大小将超过％d个字节的最大密钥长度。
...
消息：数据库'％s'的'％s'特权同时作为部分撤销和mysql.db存在。这可能意味着“ mysql”模式已损坏。
...
消息：通道'％s'的MASTER_COMPRESSION_ALGORITHMS'％s'无效。
...
消息：通道'％s'的MASTER_ZSTD_COMPRESSION_LEVEL％u无效。
...
消息：复制频道'％s'的PRIVILEGE_CHECKS_USER设置为``@`％s`，但是不允许匿名用户使用PRIVILEGE_CHECKS_USER。
...
消息：复制通道'％s'的PRIVILEGE_CHECKS_USER设置为'％s` @`％s`，但这不是现有用户。
...
消息：在复制配置存储库中找到无效的PRIVILEGE_CHECKS_USER，用于通道'％s'。使用CHANGE MASTER TO PRIVILEGE_CHECKS_USER来更正配置。
...
消息：复制通道'％s'的PRIVILEGE_CHECKS_USER设置为'％s` @`％s`，但该用户没有REPLICATION_APPLIER特权。
...
消息：不建议使用VALUES函数，并且在将来的版本中将其删除。在这种情况下，它始终返回NULL。如果要访问INSERT语句的VALUES子句中的值，请考虑使用别名（INSERT INTO ... VALUES（...）AS别名）并在ON DUPLICATE中引用alias.col而不是VALUES（col）。 KEY UPDATE子句。
...
消息：主机名不能超过％d个字符。
ER_HOSTNAME_TOO_LONG 在8.0.21中添加了。

------

错误号：3969; 符号： ER_WARN_CLIENT_DEPRECATED_PARTITION_PREFIX_KEY; SQLSTATE：HY000
消息：不赞成使用PARTITION BY KEY（）子句中具有前缀键部分'％s（％u）'的列'％s。％s。％s'，并将在以后的版本中将其删除。
ER_WARN_CLIENT_DEPRECATED_PARTITION_PREFIX_KEY 在8.0.21中添加了。

------

错误号：3970; 符号： ER_GROUP_REPLICATION_USER_EMPTY_MSG; SQLSTATE：HY000
消息：由于为恢复通道提供的用户名为空，因此START GROUP_REPLICATION命令失败。
ER_GROUP_REPLICATION_USER_EMPTY_MSG 在8.0.21中添加了。

------

错误号：3971; 符号： ER_GROUP_REPLICATION_USER_MANDATORY_MSG; SQLSTATE：HY000
消息：START GROUP_REPLICATION命令失败，因为未为恢复通道提供USER选项。
ER_GROUP_REPLICATION_USER_MANDATORY_MSG 在8.0.21中添加了。

------

错误号：3972; 符号： ER_GROUP_REPLICATION_PASSWORD_LENGTH; SQLSTATE：HY000
消息：由于为恢复通道提供的密码超过了32个字符的最大长度，因此START GROUP_REPLICATION命令失败。
ER_GROUP_REPLICATION_PASSWORD_LENGTH 在8.0.21中添加了。

------

错误号：3973; 符号： ER_SUBQUERY_TRANSFORM_REJECTED; SQLSTATE：HY000
消息：语句要求将子查询转换为非SET操作（例如IN2EXISTS或subquery-to-lateral-derived-table）。使用优化程序开关'subquery_to_derived'不允许这样做
ER_SUBQUERY_TRANSFORM_REJECTED 在8.0.21中添加了。

------

错误号：3974; 符号： ER_DA_GRP_RPL_RECOVERY_ENDPOINT_FORMAT; SQLSTATE：HY000
消息：恢复套接字端点'％s'的无效输入值。请提供有效的，逗号分隔的端点列表（IP：端口）
ER_DA_GRP_RPL_RECOVERY_ENDPOINT_FORMAT 在8.0.21中添加了。

------

错误号：3975; 符号： ER_DA_GRP_RPL_RECOVERY_ENDPOINT_INVALID; SQLSTATE：HY000
消息：服务器未在端点'％s'上侦听。只有服务器正在侦听的端点才是有效的恢复端点。
ER_DA_GRP_RPL_RECOVERY_ENDPOINT_INVALID 在8.0.21中添加了。

------

错误号：3976; 符号： ER_WRONG_VALUE_FOR_VAR_PLUS_ACTIONABLE_PART; SQLSTATE：HY000
消息：变量'％s'不能设置为'％s'的值。％s
ER_WRONG_VALUE_FOR_VAR_PLUS_ACTIONABLE_PART 在8.0.21中添加了。

------

错误号：3977; 符号： ER_STATEMENT_NOT_ALLOWED_AFTER_START_TRANSACTION; SQLSTATE：HY000
消息：在带有START TRANSACTION语句的CREATE TABLE之后，仅允许BINLOG INSERT，COMMIT和ROLLBACK语句。
ER_STATEMENT_NOT_ALLOWED_AFTER_START_TRANSACTION 在8.0.21中添加了。

------

错误号：3978; 符号： ER_FOREIGN_KEY_WITH_ATOMIC_CREATE_SELECT; SQLSTATE：HY000
消息：使用CREATE TABLE作为SELECT以及使用START TRANSACTION语句的CREATE TABLE不允许外键创建。
ER_FOREIGN_KEY_WITH_ATOMIC_CREATE_SELECT 在8.0.21中添加了。

------

错误号：3979; 符号： ER_NOT_ALLOWED_WITH_START_TRANSACTION; SQLSTATE：HY000
消息：不能使用START TRANSACTION子句％s
ER_NOT_ALLOWED_WITH_START_TRANSACTION 在8.0.21中添加了。

------

错误号：3980; 符号： ER_INVALID_JSON_ATTRIBUTE; SQLSTATE：HY000
消息：无效的json属性，错误：位置％u处的“％s”：“％s”
ER_INVALID_JSON_ATTRIBUTE 在8.0.21中添加了。

------

错误号：3981; 符号： ER_ENGINE_ATTRIBUTE_NOT_SUPPORTED; SQLSTATE：HY000
消息：存储引擎'％s'不支持ENGINE_ATTRIBUTE。
ER_ENGINE_ATTRIBUTE_NOT_SUPPORTED 在8.0.21中添加了。

------

错误号：3982; 符号： ER_INVALID_USER_ATTRIBUTE_JSON; SQLSTATE：HY000
消息：用户属性必须是有效的JSON对象
ER_INVALID_USER_ATTRIBUTE_JSON 在8.0.21中添加了。

------

错误号：3983; 符号： ER_INNODB_REDO_DISABLED; SQLSTATE： HY000
消息：由于InnoDB重做日志已禁用，因此无法执行操作。使用ALTER INSTANCE启用重做日志后，请重试
ER_INNODB_REDO_DISABLED 在8.0.21中添加了。

------

错误号：3984; 符号： ER_INNODB_REDO_ARCHIVING_ENABLED; SQLSTATE：HY000
消息：无法执行操作，因为InnoDB正在归档重做日志。请通过调用innodb_redo_log_archive_stop（）停止重做存档后重试
ER_INNODB_REDO_ARCHIVING_ENABLED 在8.0.21中添加了。

------

错误号：3985; 符号： ER_MDL_OUT_OF_RESOURCES; SQLSTATE： HY000
消息：没有足够的资源来完成锁定请求。
ER_MDL_OUT_OF_RESOURCES 在8.0.21中添加了。